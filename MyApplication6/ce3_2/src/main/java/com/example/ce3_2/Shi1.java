...
public class Shi1 extends RecyclerView.Adapter {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, final int i) {
        if (getItemViewType(i) == M) {
            ...
            ((ViewHolder) viewHolder).tv2.setText(dataBean.getTitle());
            ((ViewHolder) viewHolder).tv3.setText(dataBean.getCollect_num());
            Glide.with(context).load(dataBean.getPic()).into(((ViewHolder) viewHolder).iv2);
        } else {
            Food.DataBean dataBean = list.get(i);
